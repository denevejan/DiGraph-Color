...
GeneralVizing::GeneralVizing(const std::vector<std::vector<int>>& demand_matrix, const int T, const int ub) : demand_matrix(demand_matrix), T(T), colors(ub) {
    // initialize all colors to be free at the start
    edges_per_color = vector<vector<int>>(colors, vector<int>(T, -1));
};

vector<vector<int>> GeneralVizing::color() {
    for (int i = 0; i < T-1; i++) {
        for (int j = i+1; j < T; j++) {
            ...
            for (; d --> 0 ;) {
                ...
                // directly assigning a common free color (case 0) is strictly speaking not part of Vizing's Theorem
...
void GeneralVizing::rotateFan_case1(int i, vector<pair<int,int>>& fan) {
    int last_c = -1;
...
void GeneralVizing::rotateFan_case2(int i, vector<pair<int,int>>& fan, int x, int y) {
    // alternate xy-path
    int ii = i;
